					...
					<span style="white-space: nowrap;">
						...
						<div v-if="seen_id==0" style="display: flex;flex-direction: column;align-items: center;">
							...
							<div v-for="(each,index) in res_data1['data']" style="display: flex;flex-direction: column;align-items: center;">
								...
								<p>设备号: {{each[0]}}</p>
								<p>报警邮箱: {{res_data1["info"][1]}} </p>
								<p>更新时间: {{each[1].slice(0,10)+' ' +each[1].slice(11,19)}}</p>
								<hr style="width: 55%; size: 3em;" />

								<p>空气温度数值: {{each[2]}} <span v-html="'&nbsp;&nbsp;&nbsp;&nbsp;'"></span> 空气温度阈值: {{each[3]}}</p>
								<p>
									<span v-if="each[4]==-1">空气温度报警：关闭&nbsp;&nbsp;&nbsp;&nbsp; </span>
									<span v-else-if="each[4]==0">空气温度报警：正常&nbsp;&nbsp;&nbsp;&nbsp; </span>
									<span v-else>空气温度报警：报警&nbsp;&nbsp;&nbsp;&nbsp; 时间：{{each[5].slice(0,10)+' ' +each[5].slice(11,19)}}</span>
									</p>
									<p>
									<button v-if="each[4]==-1" class="btn btn-primary" @click="send(each[0],1)">空气温度报警部署</button>
									<button v-else class="btn btn-primary" @click="send(each[0],1)">空气温度报警解除</button> &nbsp;&nbsp;&nbsp;&nbsp;
								</p>

								<hr style="width: 55%; size: 3em;" />
								
								
								<p>空气湿度数值: {{each[6]}} <span v-html="'&nbsp;&nbsp;&nbsp;&nbsp;'"></span> 空气湿度阈值: {{each[7]}}</p>
								<p>
									<span v-if="each[8]==-1">空气湿度报警：关闭&nbsp;&nbsp;&nbsp;&nbsp; </span>
									<span v-else-if="each[8]==0">空气湿度报警：正常&nbsp;&nbsp;&nbsp;&nbsp; </span>
									<span v-else>空气湿度报警：报警&nbsp;&nbsp;&nbsp;&nbsp; 时间：{{each[9].slice(0,10)+' ' +each[9].slice(11,19)}}</span>
									</p>
									<p>
									<button v-if="each[8]==-1" class="btn btn-primary" @click="send(each[0],2)">空气湿度报警部署</button>
									<button v-else class="btn btn-primary" @click="send(each[0],2)">空气湿度报警解除</button> &nbsp;&nbsp;&nbsp;&nbsp;
								</p>
								
								<hr style="width: 55%; size: 3em;" />
								
								
								<p>土壤温度数值: {{each[10]}} <span v-html="'&nbsp;&nbsp;&nbsp;&nbsp;'"></span> 土壤温度阈值: {{each[11]}}</p>
								<p>
									<span v-if="each[12]==-1">土壤温度报警：关闭&nbsp;&nbsp;&nbsp;&nbsp; </span>
									<span v-else-if="each[12]==0">土壤温度报警：正常&nbsp;&nbsp;&nbsp;&nbsp; </span>
									<span v-else>土壤温度报警：报警&nbsp;&nbsp;&nbsp;&nbsp; 时间：{{each[13].slice(0,10)+' ' +each[13].slice(11,19)}}</span>
									</p>
									<p>
									<button v-if="each[12]==-1" class="btn btn-primary" @click="send(each[0],3)">土壤温度报警部署</button>
									<button v-else class="btn btn-primary" @click="send(each[0],3)">土壤温度报警解除</button> &nbsp;&nbsp;&nbsp;&nbsp;
								</p>
								
								<hr style="width: 55%; size: 3em;" />
								
								<p>土壤湿度数值: {{each[14]}} <span v-html="'&nbsp;&nbsp;&nbsp;&nbsp;'"></span> 土壤湿度阈值: {{each[15]}}</p>
								<p>
									<span v-if="each[16]==-1">土壤湿度报警：关闭&nbsp;&nbsp;&nbsp;&nbsp; </span>
									<span v-else-if="each[16]==0">土壤湿度报警：正常&nbsp;&nbsp;&nbsp;&nbsp; </span>
									<span v-else>土壤湿度报警：报警&nbsp;&nbsp;&nbsp;&nbsp; 时间：{{each[17].slice(0,10)+' ' +each[17].slice(11,19)}}</span>
									</p>
									<p>
									<button v-if="each[16]==-1" class="btn btn-primary" @click="send(each[0],4)">土壤湿度报警部署</button>
									<button v-else class="btn btn-primary" @click="send(each[0],4)">土壤湿度报警解除</button> &nbsp;&nbsp;&nbsp;&nbsp;
								</p>
								
								<hr style="width: 55%; size: 3em;" />
								
								<p>光强数值: {{each[18]}} <span v-html="'&nbsp;&nbsp;&nbsp;&nbsp;'"></span> 光强阈值: {{each[19]}}</p>
								<p>
									<span v-if="each[20]==-1">光强报警：关闭&nbsp;&nbsp;&nbsp;&nbsp; </span>
									<span v-else-if="each[20]==0">光强报警：正常&nbsp;&nbsp;&nbsp;&nbsp; </span>
									<span v-else>光强报警：报警&nbsp;&nbsp;&nbsp;&nbsp; 时间：{{each[21].slice(0,10)+' ' +each[21].slice(11,19)}}</span>
									</p>
									<p>
									<button v-if="each[20]==-1" class="btn btn-primary" @click="send(each[0],5)">光强报警部署</button>
									<button v-else class="btn btn-primary" @click="send(each[0],5)">光强报警解除</button> &nbsp;&nbsp;&nbsp;&nbsp;
								</p>
								...
								<p>
								<span v-if="each[22]==0 | each[22]==2">控制器: 关闭 <br> <button class="btn btn-primary" @click="send(each[0],0)">手动开启</button></span>
								<span v-else>控制器: 开启 <br> <button class="btn btn-primary" @click="send(each[0],0)">手动关闭</button></span>

								<span v-if="each[22]==2 | each[22]==3"><br> <button class="btn btn-primary" @click="send(each[0],'00')">切回自动</button></span>
						...
						<div v-if="seen_id==1" style="display: inline-block;">
							<label style="float:left">绑定设备号：</label> <input v-model="input_val[0]" style="border:0.5px solid #378888; white-space: nowrap;">
							<br>
							<label style="float:left">通知邮箱：</label> <input v-model="input_val[1]" style="border:0.5px solid #378888; white-space: nowrap;">
							<br>
							<label style="float:left">空气温度阈值：</label> <input v-model="input_val[2]" style="border:0.5px solid #378888; white-space: nowrap;">
							<br>
							<label style="float:left">空气湿度阈值：</label> <input v-model="input_val[3]" style="border:0.5px solid #378888; white-space: nowrap;">
							<br>
							<label style="float:left">土壤温度阈值：</label> <input v-model="input_val[4]" style="border:0.5px solid #378888; white-space: nowrap;">
							<br>
							<label style="float:left">土壤湿度阈值：</label> <input v-model="input_val[5]" style="border:0.5px solid #378888; white-space: nowrap;">
							<br>
							<label style="float:left">光强阈值：</label> <input v-model="input_val[6]" style="border:0.5px solid #378888; white-space: nowrap;">
							<br><br>
		
							<button class="btn btn-primary" @click="change" style="display: flex;flex-direction: column;align-items: center;">修改</button>
						</div>
