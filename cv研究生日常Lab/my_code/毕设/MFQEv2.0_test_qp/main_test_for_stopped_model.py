#main_test_for_stopped_model.py： 用stopped模型进行提取参数 -> 置零
import glob, os
import numpy as np
...
dir_PQFLabel = "Data"
dir_model = "" #no use
model_path = "stoped_model_QP32/model_step2.ckpt-32-9"
...
def sparsity(input_val,input_size,output_size):
...
def func_enhance(dir_model_pre, QP, PreIndex_list, CmpIndex_list, SubIndex_list):
    ...
    x1to2 = net_MFCNN_stoped.warp_img(BATCH_SIZE, x2, x1, False)
    x3to2 = net_MFCNN_stoped.warp_img(BATCH_SIZE, x2, x3, True)
    ...
    with tf.Session(config = config) as sess:
        ...
        val_cconv1_w = sess.run(cconv1_w)
        shape_cconv1_w = val_cconv1_w.shape
        val_cconv1_w_saprsity = sparsity(val_cconv1_w, shape_cconv1_w, shape_cconv1_w_sparsity)
        sess.run(tf.assign(cconv1_w, tf.convert_to_tensor(val_cconv1_w_saprsity)))
        # print(cconv1_w)

        val_cconv2_w = sess.run(cconv2_w)
        shape_cconv2_w = val_cconv2_w.shape
        val_cconv2_w_saprsity = sparsity(val_cconv2_w, shape_cconv2_w, shape_cconv2_w_sparsity)
        sess.run(tf.assign(cconv2_w, tf.convert_to_tensor(val_cconv2_w_saprsity)))

        val_cconv3_w = sess.run(cconv3_w)
        shape_cconv3_w = val_cconv3_w.shape
        val_cconv3_w_saprsity = sparsity(val_cconv3_w, shape_cconv3_w, shape_cconv2_w_sparsity)
        sess.run(tf.assign(cconv3_w, tf.convert_to_tensor(val_cconv3_w_saprsity)))

        val_cconv4_w = sess.run(cconv4_w)
        shape_cconv4_w = val_cconv4_w.shape
        val_cconv4_w_saprsity = sparsity(val_cconv4_w, shape_cconv4_w, shape_cconv2_w_sparsity)
        sess.run(tf.assign(cconv4_w, tf.convert_to_tensor(val_cconv4_w_saprsity)))

        val_cconv5_w = sess.run(cconv5_w)
        shape_cconv5_w = val_cconv5_w.shape
        val_cconv5_w_saprsity = sparsity(val_cconv5_w, shape_cconv5_w, shape_cconv2_w_sparsity)
        sess.run(tf.assign(cconv5_w, tf.convert_to_tensor(val_cconv5_w_saprsity)))

        val_cconv6_w = sess.run(cconv6_w)
        shape_cconv6_w = val_cconv6_w.shape
        val_cconv6_w_saprsity = sparsity(val_cconv6_w, shape_cconv6_w, shape_cconv2_w_sparsity)
        sess.run(tf.assign(cconv6_w, tf.convert_to_tensor(val_cconv6_w_saprsity)))

        val_cconv7_w = sess.run(cconv7_w)
        shape_cconv7_w = val_cconv7_w.shape
        val_cconv7_w_saprsity = sparsity(val_cconv7_w, shape_cconv7_w, shape_cconv2_w_sparsity)
        sess.run(tf.assign(cconv7_w, tf.convert_to_tensor(val_cconv7_w_saprsity)))
