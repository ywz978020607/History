...
def network2(frame1, frame2, frame3, reuse=False, scope='netflow'): # design for QP22,27,32
    ...
    with tf.variable_scope(scope, reuse=reuse):
        with slim.arg_scope([slim.conv2d],  activation_fn=tflearn.activations.prelu,
            ...
            # General CNN
            size = 20
            cc1 = slim.conv2d(c_concat, size, [3, 3], scope='cconv1')
            cc2 = slim.conv2d(cc1, size, [3, 3], scope='cconv2')
            cc3 = slim.conv2d(cc2, size, [3, 3], scope='cconv3')
            cc4 = slim.conv2d(cc3, size, [3, 3], scope='cconv4')
            cc5 = slim.conv2d(cc4, size, [3, 3], scope='cconv5')
            cc6 = slim.conv2d(cc5, size, [3, 3], scope='cconv6')
            cc7 = slim.conv2d(cc6, size, [3, 3], scope='cconv7')
