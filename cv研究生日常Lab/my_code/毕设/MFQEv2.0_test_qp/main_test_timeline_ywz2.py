...
def func_enhance(dir_model_pre, QP, PreIndex_list, CmpIndex_list, SubIndex_list):
    ...
    x1to2 = net_MFCNN_26.warp_img(BATCH_SIZE, x2, x1, False)
    x3to2 = net_MFCNN_26.warp_img(BATCH_SIZE, x2, x3, True)
    
    if QP in net1_list:
        x2_enhanced = net_MFCNN_26.network(x1to2, x2, x3to2, is_training)
    else:
        x2_enhanced = net_MFCNN_26.network2(x1to2, x2, x3to2)
...
for ite_CmpVideo in range(num_CmpVideo):
    ...
    for QP in QP_list:
        ...
        dir_model_pre = "model_QP32_v26_3"
