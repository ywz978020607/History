...
def entry_stop_gradients(target, mask):
    mask_h = tf.abs(mask - 1) #mask为零->mask_h为1
    tf.stop_gradient(mask_h * target)
    return  mask * target  #mask为0的值也不计入计算
    ...
    with tf.variable_scope(scope, reuse=reuse):
        with slim.arg_scope([slim.conv2d],  activation_fn=tflearn.activations.prelu,
            ...
            with tf.variable_scope("cconv1"):
                cconv1_mask = tf.get_variable('cconv1_mask',[3, 3, 288, 32], trainable=False)
                cc1_weights = tf.get_variable("weights",[3,3,288,32])
                cc1_bias = tf.get_variable("biases",[32])
                cc1_conv = tf.nn.conv2d(c_concat,entry_stop_gradients(cc1_weights, cconv1_mask),strides=[1,1,1,1],padding='SAME') + cc1_bias
                cc1_h_conv = tflearn.prelu(cc1_conv)

            with tf.variable_scope("cconv2"):
                cconv2_mask = tf.get_variable('cconv2_mask',[3, 3, 32, 32], trainable=False)
                cc2_weights = tf.get_variable("weights",[3,3,32,32])
                cc2_bias = tf.get_variable("biases",[32])
                cc2_conv = tf.nn.conv2d(cc1_h_conv,entry_stop_gradients(cc2_weights, cconv2_mask),strides=[1,1,1,1],padding='SAME') + cc2_bias
                cc2_h_conv = tflearn.prelu(cc2_conv)
            with tf.variable_scope("cconv3"):
                cconv3_mask = tf.get_variable('cconv3_mask',[3, 3, 32, 32], trainable=False)
                cc3_weights = tf.get_variable("weights",[3,3,32,32])
                cc3_bias = tf.get_variable("biases",[32])
                cc3_conv = tf.nn.conv2d(cc2_h_conv,entry_stop_gradients(cc3_weights, cconv3_mask),strides=[1,1,1,1],padding='SAME') + cc3_bias
                cc3_h_conv = tflearn.prelu(cc3_conv)
            with tf.variable_scope("cconv4"):
                cconv4_mask = tf.get_variable('cconv4_mask',[3, 3, 32, 32], trainable=False)
                cc4_weights = tf.get_variable("weights",[3,3,32,32])
                cc4_bias = tf.get_variable("biases",[32])
                cc4_conv = tf.nn.conv2d(cc3_h_conv,entry_stop_gradients(cc4_weights, cconv4_mask),strides=[1,1,1,1],padding='SAME') + cc4_bias
                cc4_h_conv = tflearn.prelu(cc4_conv)
            with tf.variable_scope("cconv5"):
                cconv5_mask = tf.get_variable('cconv5_mask',[3, 3, 32, 32], trainable=False)
                cc5_weights = tf.get_variable("weights",[3,3,32,32])
                cc5_bias = tf.get_variable("biases",[32])
                cc5_conv = tf.nn.conv2d(cc4_h_conv,entry_stop_gradients(cc5_weights, cconv5_mask),strides=[1,1,1,1],padding='SAME') + cc5_bias
                cc5_h_conv = tflearn.prelu(cc5_conv)
            with tf.variable_scope("cconv6"):
                cconv6_mask = tf.get_variable('cconv6_mask',[3, 3, 32, 32], trainable=False)
                cc6_weights = tf.get_variable("weights",[3,3,32,32])
                cc6_bias = tf.get_variable("biases",[32])
                cc6_conv = tf.nn.conv2d(cc5_h_conv,entry_stop_gradients(cc6_weights, cconv6_mask),strides=[1,1,1,1],padding='SAME') + cc6_bias
                cc6_h_conv = tflearn.prelu(cc6_conv)
            with tf.variable_scope("cconv7"):
                cconv7_mask = tf.get_variable('cconv7_mask',[3, 3, 32, 32], trainable=False)
                cc7_weights = tf.get_variable("weights",[3,3,32,32])
                cc7_bias = tf.get_variable("biases",[32])
                cc7_conv = tf.nn.conv2d(cc6_h_conv,entry_stop_gradients(cc7_weights, cconv7_mask),strides=[1,1,1,1],padding='SAME') + cc7_bias
                cc7_h_conv = tflearn.prelu(cc7_conv)
            ...
            cc8 = slim.conv2d(cc7_h_conv, 16, [3, 3], scope='cconv8')
