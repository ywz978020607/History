...
parser.add_argument('-es2', '--epoch_step2', type=int, default=10)  #ywz 训练次数
...
#根据权值返回对应的mask
shape_cconv1_w_sparsity = [0,0,0,20]#[3,3,288,24]
shape_cconv2_w_sparsity = [0,0,0,20]#[3,3,32,24]

#稀疏  numpy input -> output numpy
def get_sparsity_mask(input_val,input_size,output_size):
    temp_val_sum = input_val.sum(axis=0)  # output维度作和 3*3*288*32->3*288*32
    temp_val_sum = temp_val_sum.sum(axis=0)  # 3*288*32->288*32
    temp_val_sum = temp_val_sum.sum(axis=0)  # 288*32->32
    arg_temp = np.argsort(abs(temp_val_sum)) #排序的索引 从小到大
    mask = np.ones(input_size)
    delta = input_size[3] - output_size[3]
...
def main_train():
    ...
    x1to2 = net_MFCNN_stoped.warp_img(tf.shape(x2)[0], x2, x1, False)
    x3to2 = net_MFCNN_stoped.warp_img(tf.shape(x2)[0], x2, x3, True)
    ...
    ### Enhance cmp frames
    if int(QP) in net1_list:
        x2_enhanced = net_MFCNN_stoped.network(x1to2, x2, x3to2, is_training)
    else:
        x2_enhanced = net_MFCNN_stoped.network2(x1to2, x2, x3to2)
    ...
    #different restore!!!!
    # 需要把新增的层去掉(已初始化，只要去掉即可)
    # 得到该网络中，所有可以加载的参数
    variables = tf.contrib.framework.get_variables_to_restore()
    # for v in variables:
    #     print(v.name, v.shape)
    # 删除vgg_16_text层中的参数
    variables_to_restore = [v for v in variables if v.name.split(':')[0] not in ['netflow/cconv1/cconv1_mask','netflow/cconv2/cconv2_mask','netflow/cconv3/cconv3_mask','netflow/cconv4/cconv4_mask','netflow/cconv5/cconv5_mask','netflow/cconv6/cconv6_mask','netflow/cconv7/cconv7_mask']]
    # 构建这部分参数的saver
    saver = tf.train.Saver(variables_to_restore)
    saver.restore(sess, model_res_path)

    # 重新构建整体saver
    saver = tf.train.Saver(max_to_keep=None)  # define a saver

    #mask 置1 初始化
    cconv1_mask = tf.get_default_graph().get_tensor_by_name("netflow/cconv1/cconv1_mask:0")
    sess.run(tf.assign(cconv1_mask, np.ones(cconv1_mask.shape)))
    cconv2_mask = tf.get_default_graph().get_tensor_by_name("netflow/cconv2/cconv2_mask:0")
    sess.run(tf.assign(cconv2_mask, np.ones(cconv2_mask.shape)))
    cconv3_mask = tf.get_default_graph().get_tensor_by_name("netflow/cconv3/cconv3_mask:0")
    sess.run(tf.assign(cconv3_mask, np.ones(cconv3_mask.shape)))
    cconv4_mask = tf.get_default_graph().get_tensor_by_name("netflow/cconv4/cconv4_mask:0")
    sess.run(tf.assign(cconv4_mask, np.ones(cconv4_mask.shape)))
    cconv5_mask = tf.get_default_graph().get_tensor_by_name("netflow/cconv5/cconv5_mask:0")
    sess.run(tf.assign(cconv5_mask, np.ones(cconv5_mask.shape)))
    cconv6_mask = tf.get_default_graph().get_tensor_by_name("netflow/cconv6/cconv6_mask:0")
    sess.run(tf.assign(cconv6_mask, np.ones(cconv6_mask.shape)))
    cconv7_mask = tf.get_default_graph().get_tensor_by_name("netflow/cconv7/cconv7_mask:0")
    sess.run(tf.assign(cconv7_mask, np.ones(cconv7_mask.shape)))
    ...
    cconv6_w = tf.get_default_graph().get_tensor_by_name("netflow/cconv6/weights:0")
    cconv7_w = tf.get_default_graph().get_tensor_by_name("netflow/cconv7/weights:0")

    # print(sess.run(cconv1_w))
    # print(sess.run(cconv1_mask))
    ...
    for ite_step in [2]:
        ...
        for ite_epoch in range(num_epoch):
            ...
            # 修改mask
            ##retrain
            sess.run(tf.assign(cconv1_mask,
                               get_sparsity_mask(sess.run(cconv1_w), cconv1_w.shape, shape_cconv1_w_sparsity)))
            sess.run(tf.assign(cconv2_mask,
                               get_sparsity_mask(sess.run(cconv2_w), cconv2_w.shape, shape_cconv2_w_sparsity)))
            sess.run(tf.assign(cconv3_mask,
                               get_sparsity_mask(sess.run(cconv3_w), cconv3_w.shape, shape_cconv2_w_sparsity)))
            sess.run(tf.assign(cconv4_mask,
                               get_sparsity_mask(sess.run(cconv4_w), cconv4_w.shape, shape_cconv2_w_sparsity)))
            sess.run(tf.assign(cconv5_mask,
                               get_sparsity_mask(sess.run(cconv5_w), cconv5_w.shape, shape_cconv2_w_sparsity)))
            sess.run(tf.assign(cconv6_mask,
                               get_sparsity_mask(sess.run(cconv6_w), cconv6_w.shape, shape_cconv2_w_sparsity)))
            sess.run(tf.assign(cconv7_mask,
                               get_sparsity_mask(sess.run(cconv7_w), cconv7_w.shape, shape_cconv2_w_sparsity)))
            ###
            # 训练
            ...
            if ite_step == 1:
                CheckPoint_path = os.path.join(dir_model, "model_step1.ckpt-32")
            else:
                CheckPoint_path = os.path.join(dir_model, "model_step2.ckpt-32")
