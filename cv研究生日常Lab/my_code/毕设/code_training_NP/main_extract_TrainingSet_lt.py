...
def func_PatchFrame(info_patch, num_patch, ite_npy, mode):
    ...
    for ite_frame, order_frame in enumerate(range(order_FirstFrame, order_LastFrame + 1)):
        ...
        print("\rframe %d | %d" % (ite_frame + 1, num_frame_total), end="", flush=True)
    ...
    print("\rsaving 2/4...", end="", flush=True)
    random.seed(100)
    ...
    print("\rsaving 3/4...", end="", flush=True)
    random.seed(100)
    ...
    print("\rsaving 4/4...", end="", flush=True)
    random.seed(100)
    ...
    print("\r                   ", end="\r", flush=True) # clear bar
...
def main_extract_TrainingSet():
    ...
    for QP in QP_list:
        ...
        print("Available frames: %d - patches: %d" % (cal_frame, num_patch_available), flush=True)
        ...
        for ite_npy_tra in range(len(num_batch_PerNpy_list_tra)):
            ...
            print("stacking tra npy %d / %d..." % (ite_npy_tra + 1, len(num_batch_PerNpy_list_tra)), flush=True)
        ...
        for ite_npy_val in range(len(num_batch_PerNpy_list_val)):
            ...
            print("stacking val npy %d / %d..." % (ite_npy_val + 1, len(num_batch_PerNpy_list_val)), flush=True)
...
if __name__ == '__main__':
    ...
    dir_database = "D:/Desktop/IRC_Lab/2020.03/mini"
    dir_raw = dir_database + "/test_18_raw"
    dir_cmp = dir_database + "/QP42"
    dir_PQFLabel = dir_database + "/QP42/label"
    dir_save_stack_pre = "D:/Desktop/IRC_Lab/test/mini_stack_QP42"
