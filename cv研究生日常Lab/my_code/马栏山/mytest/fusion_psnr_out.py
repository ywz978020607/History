...
out_log_path = join(fusion_yuv_path, "psnr_log")
if not exists(out_log_path):
    os.system("mkdir "+out_log_path)
...
for ii in range(len(yuv_list)):
    ...
    out_log_temp_path = join(out_log_path,fusion_yuv_temp_path.split('/')[-1])
    out_log_temp_path = out_log_temp_path.replace(".yuv",".log")

    print(ori_yuv_temp_path)
    print(fusion_yuv_temp_path)
    print(out_log_temp_path)
    # vmaf_json("yuv420p",1920,1080,ori_yuv_temp_path,fusion_yuv_temp_path,out_json_path)
    cmd = "ffmpeg -s 1920x1080 -i "+fusion_yuv_temp_path+" -s 1920x1080 -i "+ori_yuv_temp_path+" -lavfi psnr=\"stats_file="+ out_log_temp_path +"\" -f null -"
    os.system(cmd)
