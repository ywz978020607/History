...
class ILI9341:
    ...
    >>> import ILI9341
    >>> from HZK16_showch import printPlay
    >>> s = open("zwcs.txt", "r")
    >>> text = s.read()
    >>> s.close()
    >>> ret = printPlay(text)
    ...
    >>> display = ILI9341.ILI9341(spi, cs=Pin(15), dc=Pin(21), rst=Pin(4))
    >>> display.fill(ILI9341.color565(0xff, 0x11, 0x22))
    >>> display.pixel(120, 160, 0)
    >>> display._render_bmp_image('django_photo_down_test.bmp', (0, 0))
    """
    ...
    def draw(self, x, y, x1, y1, color=0x0000):
        if x == x1:
            ...
            while x <= x1:
                ...
                y=y+r
    #显示中文
    def writeWords(self, rect_list):
        for i in range(len(rect_list)):
            for j in range(len(rect_list[i])):
                if rect_list[i][j]:
                    self.pixel(j*2, i*2, 0)
                    self.pixel(j*2+1, i*2, 0)
                    self.pixel(j*2, i*2+1, 0)
                    self.pixel(j*2+1, i*2+1, 0)
...
# ctrl k c, ctrl k u
    ...
    def _set_window(self, x0, y0, x1, y1):
        # Column Address Set
        self._write(0x2A)
        self._data(struct.pack('B',( (x0>>8) & 0xFF)  ))    #write_data
    ...
    def _render_bmp_image(self, filename, pos):
        ...
        with open(filename, 'rb') as f:
            ...
            #反过来画
            self._set_window(y, (width)+x, x, (height)+y)
            # self._set_window(y, (height)+y, x, (width)+x)
            # self._set_window(x, (height)+y , (width)+x, y)
            f.seek(startbit)
            while True:
                t = f.read(memread)
                if t!=b'':
                    ...
                    #     tt = t[len_t-i-1]
                    #     data += tt.to_bytes(1, byteorder='little')
                    data = t
                    self._data(data)
                else:
                    print('读完了')
...
# s = open('esp32cam_jpeg_rgb.txt', "r")
# for i in range(240):
#     for j in range(240):
#         stt = s.readline().replace("\n", "").split(' ', -1)
#         display.pixel(i, j, ILI9341.color565(int(stt[0]), int(stt[1]), int(stt[2])))
#         display.pixel(i, j, ILI9341.color565(72, 63, 56))
#display._render_bmp_image('6666.bmp', (0, 0))
