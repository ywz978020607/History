...
ap.active(True)         # 激活热点
ap.config(essid='ESP3') # 为热点配置essid（即热点名称）
...
def xiancheng1():
    ...
    while 1:
        ...
        if len(data1)>0:
            ...
            if str(last_temp)==str(data1):
                print("no new data from last")
            else:
                #如果不同，再给下一个
                #再加帧头帧尾
...
#读下一个给上一个,即便没有也要定时发=》靠线程3发送
def xiancheng2():
    aa="send from thread2"
    print("thread 2 start")
    global conn,ap,wifi,next_temp #无s1,有next_temp
...
#线程3定时发送给上一个 to last one
def xiancheng3():
    print("xiancheng3 start")
    global conn,ap,wifi,s1,next_temp  #无conn,有next_temp
    while 1:
        data3 = "start "+next_temp+" end"
        s1.send(data3.encode())
        print("send to last one ok")
        time.sleep(3)
        
_thread.start_new_thread(xiancheng1,()) 
_thread.start_new_thread(xiancheng2,()) 
_thread.start_new_thread(xiancheng3,()) 
